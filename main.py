...
class Validator:
    ...
    def __init__(self, records: Collection[Record]):   #коллекция экземпляров класса записей(Record)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Validation records')#Создаём парсер
    parser.add_argument('-input', type=str, help='Input file of data')#аргумент, который передается после опции -input, сохранится в переменную type,но прежде выполним отображение с подсказками
    parser.add_argument('-output', type=str, help='Output file of data')#аргумент, который передается после опции -output, сохранится в переменную type,но прежде выполним отображение с подсказками
    args = parser.parse_args()#После выполнения, в переменной args содержатся аргументы переданные скрипту

    data = json.load(open(args.input, encoding='windows-1251'))
    records = [Record(**el) for el in data]

    validator = Validator(records)
    valid_records = validator.processing()#Работа с функцией которая каждую запись проверяет(валидные данные которые получены после проверки)

    with open(args.output, 'w') as f:##########
        f.write(pformat([asdict(record) for record in valid_records]))

    pprint(validator.processing_log)
